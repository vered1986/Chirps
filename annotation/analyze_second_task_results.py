...
def main():
    ...
    args = docopt("""Analyze the results of the second annotation task: annotating rule types in the top 100 of different days.

        Usage: analyze_second_task_results.py <batch_result_file>
    ...
    # Load the results
    workers, results, keys_by_day = load_results(batch_result_file)
    ...
    # Compute accuracy for each day
    for day in keys_by_day.keys():

        curr_results = { key : results[key] for key in keys_by_day[day] }
        answers = compute_majority_gold(curr_results).values()
        print 'Accuracy of day %d: %.3f' % (day, np.sum(answers) * 100.0 / len(curr_results))
...
def load_results(result_file):
    ...
    return workers, worker_answers, keys_by_day
